...
class RogClient(object):
    ...
    def post_alert(self, alert: AlertObject):
        ...
        logging.info('rog post alert, type: %s, camera: [%d] \'%s\'', alert.alert_type, alert.camera_id, alert.camera_name)
    ...
    def do_auth(self):
        r = requests.post('{0}/api/v2/sessions'.format(reco_config.rogapi_url),
                            json={'session': {'email': reco_config.rogapi_username, 'password': reco_config.rogapi_password}})
    ...
    def do_post(self, alert: AlertObject):
        ...
        if not self.is_request_succeeded(r.status_code):
            logging.error("rog post alert failed, status: %d", r.status_code)
